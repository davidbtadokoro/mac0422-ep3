...
/*###########################################################################*/
PRIVATE int policy = 0; /* first fit como politica padrao */
/*###########################################################################*/
 ...
 *				setallocpol				     *
 *===========================================================================*/
PUBLIC int setallocpol(pol_num)
int pol_num;  /* numero da politica (veja include/minix/unistd.h) */
{
  if (pol_num > 3 || pol_num < 0)
    return(1);
 ...
 */
  ...
  do {
    ...
    switch (policy) {
      ...
      case 0: {
        while (hp != NIL_HOLE && hp->h_base < swap_base) {
          /* achou buraco grande o suficiente entao podemos parar */
          if (hp->h_len >= clicks) {
            candidate = hp;
      ...
      case 1: {
        cand_len = UINT_MAX;
        while (hp != NIL_HOLE && hp->h_base < swap_base) {
          /* achou candidato ao menor buraco grande o suficiente */
          if (hp->h_len >= clicks && hp->h_len < cand_len) {
            candidate = hp;
	    cand_prev_ptr = prev_ptr;
            cand_len = hp->h_len;
          }
      ...
      case 2: {
        cand_len = 0;
        while (hp != NIL_HOLE && hp->h_base < swap_base) {
          /* achou candidato ao maior buraco grande o suficiente */
          if (hp->h_len >= clicks && hp->h_len > cand_len) {
            candidate = hp;
	    cand_prev_ptr = prev_ptr;
            cand_len = hp->h_len;
    ...
    if (candidate != NIL_HOLE) {
      old_base = candidate->h_base;	/* remember where it started */
      candidate->h_base += clicks;	/* bite a piece off */
      candidate->h_len -= clicks;	/* ditto */
      ...
      if (candidate->h_len == 0) del_slot(cand_prev_ptr, candidate);
